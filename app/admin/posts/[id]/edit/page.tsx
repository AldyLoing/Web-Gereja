    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({
          ...
          categoryIds: data.categories ? data.categories.map((c: any) => c.categoryId) : []
        });
        if (data.cover) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(error.error || 'Gagal mengupdate warta');
      }
    } catch (error) {
      console.error('Error:', error);
      alert('Terjadi kesalahan saat mengupdate warta');
    } finally {
      setLoading(false);
    }
  };

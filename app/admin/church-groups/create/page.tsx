...
export default function CreateChurchGroupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/church-groups', {
        method: 'POST',
      ...
      if (response.ok) {
        alert('Kelompok sel berhasil ditambahkan!');
        router.push('/admin/church-groups');
      } else {
        const error = await response.json();
        alert(`Error: ${error.error || 'Gagal menambahkan kelompok sel'}`);
      }
    } catch (error) {
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-400 mt-1">
              Isi formulir untuk menambahkan kelompok sel baru
            </p>
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Nama Kelompok Sel <span className="text-red-500">*</span>
              </label>
              <input
                type="text"
                required

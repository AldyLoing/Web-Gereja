...
@section('page-title', 'Ulang Tahun Jemaat')

@section('content')
<div class="space-y-6">
    <!-- Filter Month -->
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
        <form action="{{ route('admin.birthdays.index') }}" method="GET" class="flex space-x-4">
            <div class="flex-1">
    ...
    <div class="bg-gradient-to-r from-yellow-400 to-yellow-500 rounded-lg shadow-lg p-6 text-white">
        <div class="flex items-center justify-between">
            <div>
                <h3 class="text-2xl font-bold">{{ $members->count() }} Jemaat</h3>
                <p class="text-yellow-100">Berulang tahun di bulan {{ $monthNames[$month] }}</p>
            </div>
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-4">
                        <div class="flex-shrink-0 w-12 h-12 bg-gradient-to-br from-green-500 to-green-600 rounded-full flex items-center justify-center text-white font-bold text-lg">
                            {{ $member->birthday_this_year->format('d') }}
                        </div>
                        <div>
                            <h4 class="text-lg font-semibold text-gray-900 dark:text-gray-100">{{ $member->name }}</h4>
                            <div class="flex items-center space-x-4 text-sm text-gray-500 dark:text-gray-400 mt-1">
                                <span>{{ $member->birthday_this_year->locale('id')->isoFormat('dddd, D MMMM YYYY') }}</span>
                                <span>•</span>
                            ...
                            @if($member->churchGroups->count() > 0)
                    ...
                    <div class="text-right">
                        @if($member->telp)
                        <a href="tel:{{ $member->telp }}" class="text-green-600 hover:text-green-700 text-sm">
                            {{ $member->telp }}
                        </a>
                        @endif
                        @if($member->email)
                        <div>
                            <a href="mailto:{{ $member->email }}" class="text-blue-600 hover:text-blue-700 text-sm">
                                {{ $member->email }}
                            </a>
                        </div>
                        @endif

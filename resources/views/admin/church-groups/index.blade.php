...
@section('title', 'Kelompok Gereja')
@section('page-title', 'Kelompok Gereja')

@section('content')
<div class="space-y-6">
    <div class="flex justify-between items-center">
        <h3 class="text-xl font-semibold text-gray-800 dark:text-gray-200">Kelompok Pelayanan</h3>
        <a href="{{ route('admin.church-groups.create') }}" class="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg font-medium transition">
            + Tambah Kelompok
        </a>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @foreach($churchGroups as $index => $group)
        @php
            $colors = [
        ...
        @endphp
        <div class="bg-gradient-to-br {{ $color }} rounded-lg shadow-lg p-6 text-white">
            <div class="flex items-start justify-between mb-4">
                <div>
                    <h4 class="text-2xl font-bold">{{ $group->name }}</h4>
                    <p class="text-white/80 text-sm mt-1">{{ $group->description }}</p>
                </div>
            ...
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-3xl font-bold">{{ $group->members_count }}</p>
                    <p class="text-white/80 text-sm">Anggota</p>
                </div>
                <div class="flex space-x-2">
                    <a href="{{ route('admin.church-groups.show', $group) }}" class="bg-white/20 hover:bg-white/30 p-2 rounded-lg transition">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                        </svg>
                    </a>
                    <a href="{{ route('admin.church-groups.edit', $group) }}" class="bg-white/20 hover:bg-white/30 p-2 rounded-lg transition">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <a href="{{ route('admin.church-groups.show', $group) }}" class="text-green-600 hover:text-green-900 dark:text-green-400 mr-3">Detail</a>
                            <a href="{{ route('admin.church-groups.edit', $group) }}" class="text-blue-600 hover:text-blue-900 dark:text-blue-400 mr-3">Edit</a>
                            <form action="{{ route('admin.church-groups.destroy', $group) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')

...
<div class="space-y-6">
        ...
        <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-lg shadow-lg p-6 text-white">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold mt-2">{{ $totalFamilies }}</p>
                </div>
                <div class="bg-white/20 p-3 rounded-lg">
                    <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-gradient-to-br from-yellow-400 to-yellow-500 rounded-lg shadow-lg p-6 text-white">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold mt-2">{{ $birthdayThisMonth }}</p>
                </div>
                <div class="bg-white/20 p-3 rounded-lg">
                    <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-lg shadow-lg p-6 text-white">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold mt-2">{{ $baptismsThisMonth }}</p>
                </div>
                <div class="bg-white/20 p-3 rounded-lg">
                    <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <!-- Monthly Baptisms -->
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-4">Baptisan Tahun {{ now()->year }}</h3>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ $group['total'] }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                            {{ $totalMembers > 0 ? number_format(($group['total'] / $totalMembers) * 100, 1) : 0 }}%
                        </td>
                    </tr>
                    @endforeach
                </tbody>
...
new Chart(churchGroupsCtx, {
    type: 'bar',
    data: {
        labels: {!! json_encode($churchGroups->pluck('name')) !!},
        datasets: [{
            label: 'Jumlah Anggota',
            data: {!! json_encode($churchGroups->pluck('total')) !!},
            backgroundColor: [
...
new Chart(baptismsCtx, {
    ...
    data: {
        ...
        datasets: [{
            label: 'Baptisan',
            data: {!! json_encode($monthlyBaptisms) !!},
            borderColor: '#009345',

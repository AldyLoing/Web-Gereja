 ...
 * Auto-generates reusable UI components
 */

import * as fs from 'fs';
import * as path from 'path';

export async function generateComponents(projectRoot: string) {
  const layoutDir = path.join(projectRoot, 'components', 'layout');
  fs.mkdirSync(layoutDir, { recursive: true });

  // Generate AdminLayout
  fs.writeFileSync(path.join(layoutDir, 'AdminLayout.tsx'), generateAdminLayout());

  // Generate PublicLayout
  fs.writeFileSync(path.join(layoutDir, 'PublicLayout.tsx'), generatePublicLayout());

  // Generate AdminSidebar
  fs.writeFileSync(path.join(layoutDir, 'AdminSidebar.tsx'), generateAdminSidebar());
}
...
function generateAdminSidebar(): string {
  return `'use client';
